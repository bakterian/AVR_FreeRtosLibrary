...
/*-----------------------------------------------------------*/
/*  Adding extra defines for the first and
 *  many times only AVR USART interface                      */
#if defined(ATMEGA_DEFAULT)
...
#if defined(__AVR_ATmega168__) || defined(__AVR_ATmega328P__) || defined(ATMEGA_DEFAULT)
...
xComPortHandle xSerialPortInitMinimal( eCOMPort ePort, uint32_t ulWantedBaud, uint16_t uxTxQueueLength, uint16_t uxRxQueueLength )
{
	...
	case USART0:
		 ...
		 * URSEL - This bit selects between accessing the UCSRC or the UBRRH Register. It is read as one when reading UCSRC.
		 * The URSEL must be one when writing the UCSRC.
		*/
		UCSR0C = (_BV(URSEL) | _BV(USBS0) | _BV(UCSZ01) | _BV(UCSZ00) );
#else
...
void xSerialPortReInit( const xComPortHandlePtr oldComPortPtr )
{
	...
	case USART0:
		...
		// Or Goldilocks Analogue uses 24.576MHz which gives 0.00% error for 9600, 19200, 38400, and 76800 baud.
#ifdef ATMEGA_DEFAULT
		ulBaudRateCounter = ( configCPU_CLOCK_HZ / ( serBAUD_DIV_CONSTANT * oldComPortPtr->baudRate ) ) - ( unsigned long ) 1;
...
ISR( USART0_RX_vect )
#else
ISR( USART_RXC_vect ) __attribute__ ((hot, flatten));
ISR( USART_RXC_vect )
#endif
{
